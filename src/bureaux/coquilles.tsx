...
function Onglet({ to, nom, compte, actif, rel }: { to: string; nom: string; compte: string; actif: boolean; rel: string }) {
  return (
    <Link
      ...
      style={{ background: actif ? rel : undefined, boxShadow: actif ? 'inset 0 1px 0 rgba(255,255,255,.05)' : undefined, fontWeight: actif ? 600 : 500, color: actif ? '#f7f7f5' : '#a3a3a0' }}
    >
      {nom}
      {compte && <span className="font-mono text-[10.5px] font-medium tabular-nums" style={{ color: actif ? '#e4e4e1' : '#9a9a97' }}>{compte}</span>}
      {actif && <span aria-hidden className="absolute bottom-0 left-2.5 right-2.5 h-[2px] bg-[#f7f7f5]" />}
    </Link>
  );
}
...
function BarreOnglets({ bureau }: { bureau: BureauKey }) {
  ...
  return (
    <nav aria-label={e.nom} className="flex h-[42px] flex-none items-stretch gap-[2px] overflow-x-auto px-[18px]" style={{ background: e.fond, borderBottom: `1px solid ${e.filet}` }}>
...
export function Pupitre() {
  ...
  return (
    <>
      ...
      <div className="flex h-[22px] flex-none items-center gap-3.5 px-[18px]" style={{ background: '#0b0b0b', borderBottom: '1px solid #1a1a1a' }} aria-label={`L’horizon : ${n} organisations, pesées par ce qui demande un humain`} role="img">
        <span className="font-mono text-[8.5px] tracking-[0.18em] text-[#9a9a97]">HORIZON</span>
        {n > 40 ? (
        ...
        <span className="font-mono text-[8.5px] tracking-[0.14em] text-[#9a9a97]">{n ? `${n} ORGANISATION${n > 1 ? 'S' : ''}` : ''}</span>
      </div>
    </>
  );
}
...
export function ConsoleColonne() {
  ...
  const entree = (cle: string, nom: string, to: string, compte: string, on: boolean) => (
    ...
    >
      <span className="w-[18px] font-mono text-[9.5px] font-medium" style={{ color: on ? '#a3a3a0' : '#9a9a97' }}>{cle}</span>
      <span className="min-w-0 flex-1 truncate whitespace-nowrap font-mono text-[12.5px]" style={{ fontWeight: on ? 600 : 500, color: on ? '#f7f7f5' : '#a3a3a0' }}>{nom}</span>
      {compte && <span className="font-mono text-[10.5px] font-medium tabular-nums" style={{ color: on ? '#e4e4e1' : '#9a9a97' }}>{compte}</span>}
      {on && <span aria-hidden className="absolute bottom-1 left-0 top-1 w-[2px] bg-[#f7f7f5]" />}
    </Link>
  );
  return (
    <nav aria-label="Console de Cyber" className="hidden w-[200px] flex-none flex-col gap-[2px] overflow-y-auto px-2.5 py-3.5 md:flex" style={{ background: e.barre, borderRight: `1px solid ${e.filet}` }}>
      <span className="px-2 pb-2.5 font-mono text-[9px] tracking-[0.2em] text-[#9a9a97]">CONSOLE</span>
      ...
      <span className="px-2 pb-2 pt-3.5 font-mono text-[9px] tracking-[0.2em] text-[#9a9a97]">PRODUITS</span>
...
export function BarreEtatCyber() {
  ...
  return (
    <div className="flex h-7 flex-none items-center gap-[22px] overflow-hidden whitespace-nowrap px-4 font-mono text-[10px] font-medium tracking-[0.1em] text-[#9a9a97]" style={{ background: e.barre, borderTop: `1px solid ${e.filet}` }} data-barre-etat>
      <span>
        POSTURE DU PARC <b className="font-semibold tabular-nums text-[#e4e4e1]">{c.parc ?? '—'}</b>
      </span>
      <span>
        ALERTES OUVERTES <b className="font-semibold text-[#e4e4e1]">{c.alertes.length}</b>
      </span>
      {/* Le critique, à l'encre : le rouge n'est dit qu'une fois par écran, et c'est l'écran qui le dit. */}
      <span>
        CRITIQUES <b className="font-semibold text-[#e4e4e1]">{critiques}</b>
      </span>
...
function Pastille({ etat }: { etat: Piece['etat'] }) {
  if (etat === 'en_ligne') return <span aria-label="en ligne" className="h-[7px] w-[7px] flex-none rounded-full bg-[#e4e4e1]" />;
  if (etat === 'chantier') return <span aria-label="chantier" className="h-[7px] w-[7px] flex-none rounded-full border border-[#8a8a87]" style={{ background: 'linear-gradient(90deg,#e4e4e1 50%,transparent 50%)' }} />;
...
export function Portes() {
  ...
  const toutes = pathname === '/studio' || (!courante && ongletDuChemin('studio', pathname)?.ecran.route === '/studio');
  const ordre: Piece[] = [];
  ...
  const porte = (to: string, contenu: React.ReactNode, on: boolean, cle: string) => (
    ...
    >
      {contenu}
      {on && <span aria-hidden className="absolute bottom-[-1px] left-2.5 right-2.5 h-[2px] bg-[#f7f7f5]" />}
    </Link>
  );
  return (
    <nav aria-label="Les pièces du Studio" className="flex h-[58px] flex-none items-stretch gap-1.5 overflow-x-auto px-[18px] py-[9px]" style={{ background: e.fond, borderBottom: `1px solid ${e.filet}` }}>
      {porte(
        '/studio',
        <>
          <Glyphe espace="studio" taille={13} couleur="#a3a3a0" />
          <span className="whitespace-nowrap text-[13px]" style={{ fontWeight: toutes ? 600 : 500, color: toutes ? '#f7f7f5' : '#c9c7c1' }}>Toutes les pièces</span>
          {studio.pieces.length > 0 && <span className="font-mono text-[10.5px] tabular-nums text-[#9a9a97]">{studio.pieces.length}</span>}
        </>,
        toutes,
        'toutes',
      )}
      {montrees.map((p) =>
        porte(
          `/studio/pieces/${p.id}`,
          <>
            <span className="border px-[5px] py-[3px] font-mono text-[9.5px] font-semibold tracking-[0.1em] text-[#9a9a97]" style={{ borderColor: e.filet }}>{p.plaque}</span>
            <span className="whitespace-nowrap text-[13px]" style={{ fontWeight: p.id === courante ? 600 : 500, color: p.id === courante ? '#f7f7f5' : '#c9c7c1' }}>{p.orgNom}</span>
      ...
      {reste > 0 && (
        <Link to="/studio" className="bx-nav flex flex-none items-center whitespace-nowrap px-3 text-[12.5px] font-medium text-[#9a9a97] hover:text-[#e4e4e1]">
...
export function Dock() {
  ...
  return (
    <nav aria-label="Le plan du mur" className="flex h-[52px] flex-none items-center gap-3.5 px-[18px]" style={{ background: e.barre, borderTop: `1px solid ${e.filet}` }}>
      <span className="hidden flex-none font-mono text-[9px] tracking-[0.2em] text-[#9a9a97] lg:inline">LE PLAN DU MUR</span>
        ...
        {CATALOGUE.strategie.onglets.map((o) => {
          ...
          return (
            <Link
              ...
              style={{ flex: PLACES_MUR[o.route] ?? 1, background: on ? e.rel : e.surf, border: `1px solid ${on ? '#44444a' : e.filet}`, fontWeight: on ? 600 : 500, color: on ? '#f7f7f5' : '#a3a3a0' }}
            >
              {o.nom}
              {on && <span aria-hidden className="absolute left-0 right-0 top-[-1px] h-[2px] bg-[#f7f7f5]" />}
            </Link>
          );
        })}
      </span>
...
export function OrganigrammeNav() {
  ...
  return (
    <>
      ...
      <div className="relative h-4 flex-none overflow-hidden" style={{ background: e.fond, borderBottom: '1px solid #161616' }} aria-hidden>
        <svg viewBox="0 0 1000 16" preserveAspectRatio="none" className="absolute inset-0 h-4 w-full">
          <path d="M0 9 H436 L442 9 L447 2 L453 15 L458 6 L462 9 H1000" fill="none" stroke="#3a3a3a" strokeWidth={1.2} vectorEffect="non-scaling-stroke" />
        </svg>
        <span data-mv className="bx-pouls absolute top-px -ml-0.5 h-1 w-1 rounded-full bg-[#a3a3a0]" style={{ left: '44.7%' }} />
      </div>
    </>
  );
}
...
export function SousOnglets({ bureau }: { bureau: BureauKey }) {
  ...
  return (
    <nav aria-label={`${o.onglet.nom} — écrans`} className="flex h-[34px] flex-none items-stretch gap-1 overflow-x-auto px-[18px]" style={{ background: e.fond, borderBottom: `1px solid ${e.filet}` }}>
      {ecrans.map((x) => {
        const on = o.ecran.route === x.route;
        return (
          <Link key={x.route} to={x.route} aria-current={on ? 'page' : undefined} className="bx-nav relative flex flex-none items-center whitespace-nowrap px-2.5 text-[12px]" style={{ fontWeight: on ? 600 : 500, color: on ? '#f7f7f5' : '#a3a3a0' }}>
            {x.nom}
            {on && <span aria-hidden className="absolute bottom-0 left-2 right-2 h-px bg-[#e4e4e1]" />}
          </Link>
        );
      })}
    </nav>
