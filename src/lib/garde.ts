...
export const garde = {
  ...
  journal: async (params: { action?: string; agent?: string; equipe?: string; org?: string; ressource?: string; since?: string; avant?: string; resultat?: GardeJournalEntree['resultat']; limit?: number; mauvais?: '1' } = {}) => (await appel<{ journal: GardeJournalEntree[] }>(`/journal${q(params)}`)).journal,
  /** Les passages des gardes ; bornée des deux côtés (une nuit), la fenêtre se lit en entier (jusqu'à 5 000). */
  rondes: async (params: { agent?: string; since?: string; avant?: string; limit?: number } = {}) => (await appel<{ rondes: GardeRonde[] }>(`/rondes${q(params)}`)).rondes,
