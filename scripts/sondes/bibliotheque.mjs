...
const grille = p.locator('button[aria-pressed], button[aria-pressed]').first();
...
const compter = () => p.locator('button[aria-pressed="true"]:not([disabled])').count();
const avant = await compter();
const cible = p.locator('button[aria-pressed="true"]:not([disabled])').first();
...
console.log(`un clic ferme « ${nomCible} » :`, avant, '→', apres, '| dialogue toujours ouvert ?', (await p.locator('button:has-text("Enregistrer et fermer")').count()) > 0);
const rouvrir = p.locator('button[aria-pressed="false"]:not([disabled])').first();
